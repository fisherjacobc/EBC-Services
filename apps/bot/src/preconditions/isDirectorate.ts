...
export class IsDirectoratePrecondition extends Precondition {
  ...
  private async checkPermission(member: GuildMember) {
    return member.roles.cache.has(Config.roleIds.coordinator) || member.roles.cache.has(Config.roleIds.governingBoard) ? this.ok() : this.error({ message: "You aren't part of the Directorate" })
  }
}
